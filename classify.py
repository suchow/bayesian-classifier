"""Classify a document according to a naive Bayes classifier."""
...
class Classify(Mode):
    ...
    def set_text(self, text):
        """Convert text to a list of word."""
    ...
    def set_file_name(self, file_name):
        try:
            ...
            raise ValueError(
                'Unable to read file "%s" because %s'.format(file_name, e))
    ...
    def validate(self, args):
        if len(args) != 5:
            raise ValueError(
                'Usage: %s classify <file> <doctype> <doctype>' % args[0])
    ...
    def output(self, result):
        print 'Probability that document is %s rather than %s is %1.2f' %\
            (self.doctype1, self.doctype2, result)
